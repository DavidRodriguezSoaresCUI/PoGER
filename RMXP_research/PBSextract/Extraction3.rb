...
module RPG
  class Tileset
    ...
    def toString()
      return sprintf("{ \"_class\":\"RPG::Tileset\", \"id\":%d, \"name\":\"%s\", \"tileset_name\":\"%s\", \r\n\"passages\":%s, \r\n\"priorities\":%s, \r\n\"terrain_tags\":%s }",
        @id,
        @name,
        @tileset_name,
        self.table2str(@passages),
        self.table2str(@priorities),
...
def SaveMaps()
  ...
  for id in 1...76
    ...
    file_name = sprintf( "%03d_%s", id, map_name.gsub(' ','_').gsub('\\','') )
    File.open( sprintf("%s/%s.txt", 'Maps', file_name ),"wb"){|f|
      # UTF-8 BOM
      writeBOM( f )
    ...
    #$observer=[]
    File.open( sprintf("%s/%03d.events", 'Maps', id),"wb"){|f|
      # UTF-8 BOM
      writeBOM( f )
...
def SaveTilesets()
  ...
  Dir.mkdir('Tileset_data') rescue Exception
  ...
  $data_tilesets.each { |t|
    ...
    name = t.name.gsub(' ','_').gsub('\\','')
    if name==""
      $observer.push("Unnamed tileset found. Skipping..")
      next
    else
      $observer.push(t.name)
    end
    
    File.open( sprintf("Tileset_data/%s", name ), "wb"){|f|
      # UTF-8 BOM
      writeBOM( f )
